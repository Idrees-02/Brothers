...
class InstallationInvoiceForm(QWidget):
    ...
    def __init__(self, conn: sqlite3.Connection, user: sqlite3.Row, parent=None):
        ...
        contact_row.addLayout(_labeled("العنوان (اختياري)", self.address_input), 1)
        contact_row.addLayout(_labeled("المنطقة *", self.area_region_input), 1)
        form.addLayout(contact_row)

        # Row 3: installation/tax toggles, spread edge-to-edge.
        ...
        toggles_row.addWidget(self.with_installation_checkbox, 1)
        ...
        # Last row: discount, deposit, payment method, and the installation
        # date - spread edge-to-edge below the items table (and its
        # add/remove buttons).
        ...
        _expand(self.installation_date_input)
        ...
        billing_row.addLayout(_labeled("المقدم (يُدفع عند الحجز)", self.deposit_input), 1)
        billing_row.addLayout(_labeled("طريقة دفع المقدم *", self.payment_method_combo), 1)
        billing_row.addLayout(_labeled("تاريخ التركيب", self.installation_date_input), 1)
