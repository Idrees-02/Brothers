...
number), editing a previously saved one, a date-range filter over the list
below, and printing a single voucher or the currently-filtered list."""
...
class ExpenseFormScreen(QWidget):
    def __init__(self, conn: sqlite3.Connection, user: sqlite3.Row, parent=None):
        ...
        subtitle = QLabel("تسجيل سند صرف أو مصروف عام جديد")
        ...
        self.save_button = QPushButton("حفظ سند الصرف")
    ...
    def _do_save(self, then_print: bool) -> None:
        ...
        try:
            if self._browsed_id is None:
                new_id = voucher_service.create_expense(
                    self._conn,
                    self._user,
                ...
                voucher_service.update_expense(
                    self._conn,
                    self._user,
                    self._browsed_id,
                    description=self.description_input.text().strip(),
                    ...
                    expense_date=self.date_input.date().toString("yyyy-MM-dd"),
                    account_id=account_id,
                    override_password_prompt=lambda: prompt_override_password("تعديل سند صرف", self),
                    note=self.note_input.text().strip() or None,
                )
    ...
    def _refresh_next_number_preview(self) -> None:
        next_no = settings_repo.preview_next_number(self._conn, "voucher", "expense")
        self.navigator.set_current_number(next_no)
        has_any = vouchers_repo.get_most_recent_expense_id(self._conn) is not None
    ...
    # -------------------------------------------------------- navigation
    def _load_voucher(self, expense_id: int) -> None:
        row = vouchers_repo.get_expense(self._conn, expense_id)
        if row is None:
            return
        self._browsed_id = expense_id
        ...
        def apply():
            ...
            self.date_input.setDate(QDate.fromString(row["expense_date"], "yyyy-MM-dd"))
        ...
        self.navigator.set_navigation_enabled(
            vouchers_repo.get_adjacent_expense_id(self._conn, expense_id, "previous") is not None,
            vouchers_repo.get_adjacent_expense_id(self._conn, expense_id, "next") is not None,
        )
    ...
    def _start_new_voucher(self) -> None:
        ...
        self.save_button.setText("حفظ سند الصرف")
        self.new_voucher_button.setEnabled(False)
        self._reset_form()
        self._dirty_tracker.mark_clean()
        self._refresh_next_number_preview()
    ...
    def _go_previous(self) -> None:
        ...
        if self._browsed_id is None:
            ...
            most_recent = vouchers_repo.get_most_recent_expense_id(self._conn)
            if most_recent is not None:
                self._load_voucher(most_recent)
            return
        adjacent = vouchers_repo.get_adjacent_expense_id(self._conn, self._browsed_id, "previous")
    ...
    def _go_next(self) -> None:
        if self._browsed_id is None or not self._dirty_tracker.confirm_discard():
            return
        adjacent = vouchers_repo.get_adjacent_expense_id(self._conn, self._browsed_id, "next")
        if adjacent is not None:
            self._load_voucher(adjacent)
        else:
    ...
    def _jump_to_number(self, voucher_no: str) -> None:
        if not voucher_no or not self._dirty_tracker.confirm_discard():
            return
        row = vouchers_repo.get_expense_by_voucher_no(self._conn, voucher_no)
        if row is None:
            QMessageBox.warning(self, "غير موجود", f"لا يوجد سند صرف برقم {voucher_no}")
            return
    ...
    def _filter_by_date(self) -> None:
        ...
        self._populate(vouchers_repo.list_expenses(self._conn, start, end))

    def _refresh_table(self) -> None:
        self._populate(vouchers_repo.list_expenses(self._conn))
    ...
    def _populate(self, rows: list[sqlite3.Row]) -> None:
        ...
        for i, row in enumerate(rows):
            ...
            self.table.setItem(i, 4, QTableWidgetItem(row["expense_date"]))

    # ------------------------------------------------------------ print
    def _print_voucher(self, expense_id: int) -> None:
        row = vouchers_repo.get_expense(self._conn, expense_id)
        if row is None:
            return
        account_names = {a["id"]: a["name"] for a in accounts_repo.list_accounts(self._conn, include_inactive=True)}
